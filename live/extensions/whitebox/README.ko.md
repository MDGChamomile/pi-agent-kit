...
GitHub 호스팅 실행기는 Whitebox에 필요한 네트워크 네임스페이스 작업을 허용하지 않습니다. 따라서 CI는 정책과 확장 연결을 검사하는 13개 테스트를 `npm run test:portable`로 실행하며, 지원되는 Linux 호스트에서는 `npm test` 전체 통합 검사를 수행해야 합니다.
